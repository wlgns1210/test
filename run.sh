...
API_INFO=$(RATE="${RATE}" python3 << 'PYEOF'
import json, sys, os
...
total_weight = sum(a.get('weight', 1) for a in apis)
...
for i, api in enumerate(apis, 1):
    ...
    erps    = rate * w / total_weight
    suffix  = f'  ({name})' if name else ''
    print(f"    {i}. [{method}] {url} → {status}  weight:{w} ({pct:.0f}%  ~{erps:.1f} req/s){suffix}")
PYEOF
)
...
# 첫 줄 = API 개수, 둘째 줄 = 총 weight, 나머지 = 출력용 텍스트
URL_COUNT=$(echo "$API_INFO" | sed -n '1p')
TOTAL_WEIGHT=$(echo "$API_INFO" | sed -n '2p')
API_LIST=$(echo "$API_INFO" | tail -n +3)
...
echo "╚════════════════════════════════════════════════════════╝"
echo ""
echo "  Peak RPS      : ${RATE} req/s 합계  (~$((RATE * 3600)) req/h)"
echo ""
echo "  대상 API (합계 ${RATE} req/s, 가중치 분배):"
echo "${API_LIST}"
...
# ── 3. k6 실행 ────────────────────────────────────────────────
echo "🚀  k6 부하 테스트 시작  [${PATTERN^^}]  (합계 ${RATE} req/s, ${URL_COUNT}개 API 가중치 분배)..."
echo ""
k6 run \
  --out "${INFLUX_OUT}" \
  -e RATE="${RATE}" \
