...
# 첫 줄 = API 개수, 둘째 줄 = 총 RPS, 나머지 = 출력용 텍스트
URL_COUNT=$(echo "$API_INFO" | sed -n '1p')
TOTAL_RPS=$(echo "$API_INFO" | sed -n '2p')
...
echo "╚════════════════════════════════════════════════════════╝"
echo ""
echo "  Peak RPS      : ${RATE} req/s × ${URL_COUNT}개 API = 총 ${TOTAL_RPS} req/s  (~$((TOTAL_RPS * 3600)) req/h)"
echo ""
...
# ── 3. k6 실행 ────────────────────────────────────────────────
echo "🚀  k6 부하 테스트 시작  [${PATTERN^^}]  (${URL_COUNT}개 API × ${RATE} req/s = 총 ${TOTAL_RPS} req/s)..."
echo ""
k6 run \
  --out "${INFLUX_OUT}" \
  -e RATE="${RATE}" \
