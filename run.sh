...
#    URL2=http://host2/v1/api URL3=http://host3/v1/api ./run.sh  # 3개 API 동시 부하
#    PATTERN=spike ABNORMAL_RATE=30 RATE=100 ./run.sh          # 패턴 + 비정상 혼합
#    ./run.sh stop                                             # 스택 종료
# ─────────────────────────────────────────────────────────────
...
ABNORMAL_RATE="${ABNORMAL_RATE:-0}"  # 비정상 요청 비율 (0~100 %)
...
if [[ "${URL_COUNT}" -gt 1 ]]; then
  echo "  대상 URL (${URL_COUNT}개 독립 부하 — 각각 ${RATE} req/s):"
  echo "    URL1 : ${URL}"
  [[ -n "${URL2}" ]] && echo "    URL2 : ${URL2}"
  [[ -n "${URL3}" ]] && echo "    URL3 : ${URL3}"
else
  echo "  URL           : ${URL}"
fi
...
# ── 3. k6 실행 ────────────────────────────────────────────────
echo "🚀  k6 부하 테스트 시작  [${PATTERN^^}]  (API ${URL_COUNT}개 × ${RATE} req/s 독립 부하)..."
echo ""
k6 run \
  --out "${INFLUX_OUT}" \
  -e URL="${URL}" \
